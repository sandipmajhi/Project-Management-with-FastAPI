...
async def AssignProjecttoEmpa(assign: Assign, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        project_availability = conn.local.project.find_one(ObjectId(assign.project))
        ...
        assign_availability = conn.local.assign.find_one({"empa_user":assign.empa_user,"project":assign.project})
        print(assign_availability)
        if assign_availability is not None:
            return JSONResponse(content={"message":"Project is already assigned to this user"}, status_code=status.HTTP_400_BAD_REQUEST)

        # # If the project is already assigned to some user
        # project_is_assigned = conn.local.assign.find_one({"project": assign.project})
        ...
        return JSONResponse(content={"message": "Project not Assigned"}, status_code=status.HTTP_400_BAD_REQUEST)
